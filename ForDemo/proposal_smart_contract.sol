...
contract DepositandWithdraw { 
    ...
    uint public TotalValueAllocatedForTheProposalContract;
    ...
    address[] public validatorsWhoApproved;
	...
	function addValidators(address _voterAddress, string memory _voterOfficialName) public inState(State.Created) onlySuperAdmin{
		voter memory v;
			    ...
			    v.choice = _choice;
			        if(_choice){
			            ...
			            found = true;	
                            ...
                            if(_choice == true){
                                validatorsWhoApproved.push(msg.sender);
                            }
                                if(_choice == false){
    ...
    function withdrawAllTokenFromThePool(uint256 amount, bytes32 symbol, uint _i) FounderOnlyWithdraw external {
    
        require(validatorsWhoApproved.length >= 3,"the validation of approval has not exceeded 3");

        if(validatorsWhoApproved.length <= 3 && validatorsWhoApproved[_i] == address(0)){
            revert("SuperAdmin has rejected one of the validators submission, founder has been denied to withdraw funds");
        }
        ...
        require(accountBalances[msg.sender][symbol] >= amount, 'Insufficent funds');
            ...
            if(proposalCancelledRevertWithdrawlToInvestors == false){
                ...
                if(proposalCancelledRevertWithdrawlToInvestors == true){
                    if(validatorsWhoApproved.length > 3){
    ...
    function Withdraw10PercentOfSingleTokenDeposit(bytes32 symbol) inState(State.HugeDepositor) FounderOnlyWithdraw public{
    ...
    // function SuperAdminDecision(uint i, address _ap, bool _choice) public onlySuperAdmin {
    
    //     if(validatorsWhoApproved[i] == _ap && _choice == false){
    //         validatorsWhoApproved[i] = address(0);
    //         _ap = address(0);
    //         calculatingAddress0.push(_ap);
    //         ValidatorWhoApprovedMap[msg.sender] = false;
    //     }

    //      if(validatorsWhoApproved[i] == _ap && _choice == true){
    //         validatorsWhoApproved[i] = _ap;
    //         calculatingAddress0.push(_ap);
    //         ValidatorWhoApprovedMap[msg.sender] = true;
    //     }
    // }

    function SuperAdminReviewValidatorRequest(uint _i,address _ad, bool _choice) public onlySuperAdmin{

		// validator should previously know who he is going to disapprove or apporve the choice
		if(validatorsWhoApproved[_i] == _ad){
           validatorWhoRejected.push(_ad);    
		   delete validatorsWhoApproved[_i];   
            // This makes the address as 0x00 but the position value is still 0 or 1
		}
	}
