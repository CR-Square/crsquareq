...
contract DepositandWithdraw { 
	...
	function addValidators(address _voterAddress, string memory _voterOfficialName) public  onlySuperAdmin{
		voter memory v;
    ...
    function depositFoundersToken(uint256 amount, bytes32 symbol) external FounderOnlyWithdraw{
        ERC20(whitelistedTokens[symbol]).transferFrom(msg.sender, address(this), amount);
        totalFoundersToken[msg.sender] = amount;
        totalDepositedFounderTokenInPot += amount;
    }

    function transferFounderTokenToInvestors(uint256 amount,address _investor, bytes32 symbol) FounderOnlyWithdraw external{
        totalDepositedFounderTokenInPot -= amount;
        ERC20(whitelistedTokens[symbol]).transfer(_investor, amount);
    }

    function withdrawAllTokenFromThePool(uint256 amount, uint _i, bytes32 symbol) FounderOnlyWithdraw external {
