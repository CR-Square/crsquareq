...
// Method: LINEAR
    ...
    */
    ...
    mapping(uint => uint) private dates;

    function testDue(due[] memory _dues) public {
        for(uint i = 0; i < _dues.length; i++){
            dates[_dues[i]._dateDue] = _dues[i]._fundDue;
        }
    }

    function readDate(uint _date) public view returns(uint){
        return dates[_date];
    }  
    ...
    function setNonLinearInstallments(address _founder, address _founderSmartContractAd, uint _vestId, address _investor, due[] memory _dues) public {
        ...
        if(f.verifyFounder(_founder) == true){
            uint duesAmount;
            for(uint i = 0; i < _dues.length; i++){     // error with for loop
                vestingDues[_vestId][_investor]._date[i+1] = _dues[i]._dateDue;
                vestingDues[_vestId][_investor]._status[i+1] = false;
                vestingDues[_vestId][_investor]._fund[i+1] = _dues[i]._fundDue;
                duesAmount += _dues[i]._fundDue;
            }
