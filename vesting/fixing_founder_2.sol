...
// SUBSEQUENT ID SETUP:
    ...
    // INVESTOR DEPOSIT:
    function depositStableTokens(address _investor,address _founder, uint256 amount, bytes32 symbol,address tokenAddress, uint _initialId, uint _subsId) external {
    ...
    // FOUNDER DEPOSIT:
    function depositFoundersToken(address _founder, uint256 amount, bytes32 symbol, address tokenAddress, uint _initialId, uint _subsId) external {
    ...
    function withdrawAllStableCoinFromThePool(address _founder,address _investor,  uint256 amount, bytes32 symbol, uint _subsId) external  returns(bool condition){
        bool status = false;
        ...
        // subsequentBalanceOfInvestor[investor][_subsId] -= amount;
    ...
    function withdrawAllFounderTokenFromThePool(address _founder, address _investor, uint256 amount, bytes32 symbol, uint _subsId) external  returns(bool condition) {
        bool status = false;
        require(arrApprovedValidator[_subsId].length >= 3,"maximum validators has not voted yet");
        if(arrApprovedValidator[_subsId].length >= 3){
            status = true;
    ...
    function Withdraw10PercentOfStableCoin(address _investor, address _founder, bytes32 symbol, uint _initialId, uint _subsId) public  {
        if(initialfounderId[_initialId][_founder] == msg.sender){
            subsequentBalanceOfInvestor[_investor][_subsId] -= initialBalanceOfInvestor[_investor][_initialId];
    ...
    function Withdraw10PercentOfFounderToken(address _founder, address _investor, bytes32 symbol, uint _initialId, uint _subsId) public  {
