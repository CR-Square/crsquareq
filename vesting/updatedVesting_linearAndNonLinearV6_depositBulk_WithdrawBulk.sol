...
// Method: LINEAR
    ...
    */
    ...
    function depositFounderLinearTokensToInvestors(address _founder, address _founderSmartContractAd, bytes32 _symbol, uint _vestId, uint _tgeDate, uint _tgePercent, uint _vestingStartDate, uint _vestingMonths, investors[] memory _investors) public {
        require(msg.sender == _founder,"The connected wallet is not founder wallet");
        Founder f = Founder(_founderSmartContractAd);   // Instance from the founder smart contract. 
        require(f.verifyFounder(_founder) == true,"The founder is not registered yet");
        uint totalTokens = 0;
        for(uint i =0; i < _investors.length; i++){
            address _investor = _investors[i]._investor;
            uint _amount = _investors[i]._tokens * (10**18);
            totalTokens += _amount;
            ...
            2. makes this available for the investor to withdraw after "_tgeDate".
            */
            vs[msg.sender].tgeFund[_vestId][_investor] = ( vs[msg.sender].tgePercentage[_vestId][_investor] * vs[msg.sender].depositsOfFounderTokensToInvestor[_vestId][_investor]) / 100;
            /*REMAININGFUND:
            1. This will divide the fund based on installments.
            */
    ...
    */
    ...
    function setNonLinearInstallments(address _founder, address _founderSmartContractAd, uint _vestId, address _investor, due[] memory _dues) public {
        ...
        if(f.verifyFounder(_founder) == true){
            ...
            for(uint i = 0; i < _dues.length; i++){     // error with for loop status: resolved.
                vestingDues[_vestId][_investor]._date[i+1] = _dues[i]._dateDue;
                vestingDues[_vestId][_investor]._status[i+1] = false;
                vestingDues[_vestId][_investor]._fund[i+1] = _dues[i]._fundDue * (10**18);  // added the 10 ** 18 condition here.
                duesAmount += _dues[i]._fundDue * (10**18);
            }
    ...
    function depositFounderNonLinearTokens(address _founder, address _founderCoinAddress, address _founderSmartContractAd, bytes32 _symbol, uint _vestId, uint _amount, address _investor, uint _tgeDate, uint _tgePercent) public{
        require(msg.sender == _founder,"The connected wallet is not founder wallet");
        Founder f = Founder(_founderSmartContractAd);   // Instance from the founder smart contract. 
        uint _tgePercentage;
        uint _founderDeposit;
