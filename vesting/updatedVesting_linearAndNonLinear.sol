...
// Method: LINEAR
    ...
    function withdrawLinearInstallmentAmount(address _investor,address _founder, uint _vestId, uint _index, bytes32 _symbol) public {
        require(msg.sender == _investor,"The connected wallet is not investor wallet");
        uint amt;
    ...
    */
    ...
    function withdrawNonLinearInstallmentAmount(address _investor,address _founder, uint _vestId, uint _index, bytes32 _symbol) public {
        require(msg.sender == _investor,"The connected wallet is not investor wallet");
        uint amt;
        if(block.timestamp >= vestingDues[_vestId][_investor].date[_index]){
            if(vestingDues[_vestId][_investor].status[_index] != true){
